...
ClassImp(TStripLineFitOld)

//-----------------------------------------------------------------------------
double TStripLineFitOld::PulseShape(double* x, double* par) {
  Double_t f(0), dx;
...
//-----------------------------------------------------------------------------
void TStripLineFitOld::FCN(Int_t&    NPar , 
			 Double_t* Gin  , 
			 Double_t& F    , 
			 Double_t* Par  , 
			 Int_t     IFlag) {
...
//-----------------------------------------------------------------------------
  ...
  for (int i=min_bin; i<=max_bin; i++) {
    ...
    f     = TStripLineFitOld::PulseShape(&t,Par);
...
//-----------------------------------------------------------------------------
TStripLineFitOld::TStripLineFitOld(const char* Name, const char* Title, TObjArray* ListOfChannels) : 
  ...
  fNPar   = 1;
  fMinuit = new TMinuit();
  fMinuit->SetFCN(TStripLineFitOld::FCN);
...
//   fChi2Max[1] = 30.;
  ...
  for (int i=0; i<fNChannels; i++) {
    ...
    res->fFun = new TF1(Form("f_gaus_ch_%03i",i),TStripLineFitOld::PulseShape,0,1024,fNPar);
...
//-----------------------------------------------------------------------------
TStripLineFitOld::~TStripLineFitOld() {
}
...
// Level : 0.1, 0.2 etc...
//----------------------------------------------------------------------------- 
double TStripLineFitOld::T0(TDrsFitResults* Res, double Level) {
...
//-----------------------------------------------------------------------------
double TStripLineFitOld::T0CFD(TDrsFitResults* Res, double Level) {
...
//-----------------------------------------------------------------------------
double TStripLineFitOld::Dt(TDrsFitResults* R1, TDrsFitResults* R2, double Level) {
...
//-----------------------------------------------------------------------------
double       TStripLineFitOld::DtCFD(TDrsFitResults* R1, TDrsFitResults* R2, double Level) {
  double t1l, t2l, dtl;
...
//-----------------------------------------------------------------------------
int TStripLineFitOld::BookHistograms() {
...
//-----------------------------------------------------------------------------
int TStripLineFitOld::FillHistograms() {
  ...
  ich1 = GetChannel(0)->GetNumber(); // GetTimeChannelNumber(0);
  ich2 = GetChannel(1)->GetNumber(); // GetTimeChannelNumber(1);
...
//     printf(" >>>> TStripLineFitOld entry = %i   dt = %f\n",
...
//   }
  ...
  TDrsChannel* ch1 = GetChannel(1);
				// select photopeak in the second channel

  if ((ch1->GetQ1() > 420) && ( ch1->GetQ1() < 600)) {
    Hist->fDt[1]->Fill(dt);
  }
...
// Gaussian fit
//-----------------------------------------------------------------------------
int TStripLineFitOld::Fit(const TDrsChannel* Channel) { 
...
//  max_bin = Data->GetMax1Cell();
  ...
  for (int i=min_bin; i<ncells; i++) {
    max_bin = i;
    if ((Channel->fV[i] > fMaxFraction[ich]*vmax) && (Channel->fV[i+1] < Channel->fV[i])) {
      break;
    }
  }
...
// Minuit 
//-----------------------------------------------------------------------------
//  TDrsFitAlg::fgFitData    = (TTofData*) Data;
...
//    Data->fHistShape->Fit(res->fFun,"w0q","",min_bin-0.5,max_bin);
    ...
    Channel->fHist.fShape->Fit(res->fFun,"w0q","",min_bin-0.5,max_bin+0.5);
 
  //  TF1* f_fit = Data->fHistShape->GetFunction(f->GetName());

    TF1* f_fit = Channel->fHist.fShape->GetFunction(res->fFun->GetName());
