...
#include "drs/ana/TSplineFitAlgNew.hh"
...
ClassImp(TSplineFitAlgNew)

TSpline* TSplineFitAlgNew::fgSpline;

//Test Area -------------------------------------------------------------------

TSpline* TSplineFitAlgNew:: GetSpline() {
  TFile* f1    = TFile::Open("Comparison_1.root");
  TCanvas* c11 = (TCanvas*) f1->Get("Canvas_1_n2");
  TH1* h1      = (TH1*) c11->GetListOfPrimitives()->FindObject("ave_pulse_00_pfx");
  TSpline* spln = new TSpline3(h1,"");
  return (spln);
}


//-----------------------------------------------------------------------------
Double_t TSplineFitAlgNew::PulseShape(double* x, double* par) {
  double f, dx;
...
// OR
// dx = x[0]-par[0];
// One or the other module negative sign

  dx = par[0]-x[0]; 
  f  = TSplineFitAlgNew::fgSpline->Eval(dx);

  return f;
}

//_____________________________________________________________________________
void TSplineFitAlgNew::FCN(Int_t&    NPar , 
			 Double_t* Gin  , 
			 Double_t& F    , 
			 Double_t* Par  , 
			 Int_t     IFlag) {
...
//-----------------------------------------------------------------------------
  ...
  for (int i=min_bin; i<=max_bin; i++) {
    ...
    f     = TSplineFitAlgNew::PulseShape(&t,Par);
...
//-----------------------------------------------------------------------------
TSplineFitAlgNew::TSplineFitAlgNew(const char* Name, const char* Title, int NChannels) : 
  ...
  fNPar   = 3;
  fMinuit = new TMinuit();
  fMinuit->SetFCN(TSplineFitAlgNew::FCN);
  ...
  for (int i=0; i<NChannels; i++) {
    ...
    res->fFun = new TF1(Form("f_gaus_ch_%03i",i),TSplineFitAlgNew::PulseShape,0,1024,fNPar);
 ...
 TSplineFitAlgNew::fgSpline = TSplineFitAlgNew::GetSpline();
}

//-----------------------------------------------------------------------------
TSplineFitAlgNew::~TSplineFitAlgNew() {
}


//----------------------------------------------------------------------------
// calculate derivative at a given 'Level', extrapolate down to zero to get the time stamp
...
// Level : 0.1, 0.2 etc...
//----------------------------------------------------------------------------- 
double TSplineFitAlgNew::T0(TDrsFitResults* Res, double Level) {
...
//-----------------------------------------------------------------------------
double TSplineFitAlgNew::T0CFD(TDrsFitResults* Res, double Level) {
...
//-----------------------------------------------------------------------------
double TSplineFitAlgNew::Dt(TDrsFitResults* R1, TDrsFitResults* R2, double Level) {
...
//-----------------------------------------------------------------------------
double       TSplineFitAlgNew::DtCFD(TDrsFitResults* R1, TDrsFitResults* R2, double Level) {
...
//-----------------------------------------------------------------------------
int TSplineFitAlgNew::BookHistograms() {
...
//-----------------------------------------------------------------------------
int TSplineFitAlgNew::FillHistograms() {
...
// Gaussian fit
//-----------------------------------------------------------------------------
int TSplineFitAlgNew::Fit(const TTofData* Data) { 
...
//-----------------------------------------------------------------------------
    ...
    res->fFun->SetParameter(2,4.);
    res->fFun->SetParLimits(2,2,10.);
    //  f->SetParError(2,0.);
...
//    Data->fHistShape->Fit(res->fFun,"w0q","",min_bin-2.5,max_bin+0.5);
    Data->fHistShape->Fit(res->fFun,"w0q","",min_bin-2.5,max_bin);
