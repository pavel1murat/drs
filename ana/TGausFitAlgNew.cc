...
ClassImp(TGausFitAlgNew)

//-----------------------------------------------------------------------------
double TGausFitAlgNew::PulseShape(double* x, double* par) {
  double f, dx;
...
void TGausFitAlgNew::FCN(Int_t&    NPar , 
			 Double_t* Gin  , 
			 Double_t& F    , 
			 Double_t* Par  , 
			 Int_t     IFlag) {
...
//-----------------------------------------------------------------------------
  ...
  for (int i=min_bin; i<=max_bin; i++) {
    ...
    f     = TGausFitAlgNew::PulseShape(&t,Par);
...
//-----------------------------------------------------------------------------
TGausFitAlgNew::TGausFitAlgNew(const char* Name, const char* Title, int NChannels) : 
  ...
  fNPar   = 3;
  fMinuit = new TMinuit();
  fMinuit->SetFCN(TGausFitAlgNew::FCN);
  ...
  for (int i=0; i<fNChannels; i++) {
    ...
    res->fFun = new TF1(Form("f_gaus_ch_%03i",i),TGausFitAlgNew::PulseShape,0,1024,fNPar);
    fListOfFitResults->AddAt(res,i);
  }
  
  for (int i=0; i<fNTrigChannels; i++) {
    SetMinMaxTrigFractions(i,0.02,0.5);
    res       = new TDrsFitResults(i);
    res->fFun = new TF1(Form("f_gaus_tr_%03i",i),TGausFitAlgNew::PulseShape,0,1024,fNPar);
...
// Level : 0.1, 0.2 etc...
//----------------------------------------------------------------------------- 
double TGausFitAlgNew::T0(TDrsFitResults* Res, double Level) {
...
//-----------------------------------------------------------------------------
double TGausFitAlgNew::T0CFD(TDrsFitResults* Res, double Level) {
...
//-----------------------------------------------------------------------------
double TGausFitAlgNew::Dt(TDrsFitResults* R1, TDrsFitResults* R2, double Level) {
...
//-----------------------------------------------------------------------------
double       TGausFitAlgNew::DtCFD(TDrsFitResults* R1, TDrsFitResults* R2, double Level) {
...
//-----------------------------------------------------------------------------
int TGausFitAlgNew::BookHistograms() {
  ...
  for (int i=0; i<10; i++) {
    sprintf(name,"dtc[%i]",i);
    sprintf(title,"dtc[%i]",i);
    HBook1F(Hist->fDtc[i],name,title,500,-5,5);
  }
  ...
  for (int i=0; i<fNChannels; i++) {
    ...
    if (ich >= 0) {
      ...
      HBook1F(Hist->fChi2[i],name,name,200,0,20);
...
//       HBook1F(Hist->fBaseOffset[i],name,name,100,-0.5,0.5);
      ...
      HBook1F(Hist->fT0Fit[i],name,name,500,0,550);
  ...
  HBook2F(Hist->fDtcVsDfc,"dtc_vs_dfc","Dtc Vs D(first Cell)",50,-25,25,250,-5,5);

  HBook2F(Hist->fDt2VsDfc_24,"dt2_vs_dfc_24","Dt2_24 Vs D(first Cell)",50,-25,25,250,-5,5);
  HBook2F(Hist->fDtcVsDfc_24,"dtc_vs_dfc_24","Dtc_24 Vs D(first Cell)",50,-25,25,250,-5,5);
  ...
  HBook2F(Hist->fDtcVsIch1,"dtc_vs_ich1","Dtc Vs Ich1",50,0,50,250,-5,5);
  HBook2F(Hist->fDtcVsIch2,"dtc_vs_ich2","Dtc Vs Ich2",50,0,50,250,-5,5);

  HBook2F(Hist->fDt2VsIch1_24,"dt2_vs_ich224","Dt2 Vs Ich1, ICH2=24",50,0,50,250,-5,5);
  HBook2F(Hist->fDtcVsIch1_24,"dtc_vs_ich224","Dtc Vs Ich1, ICH2=24",50,0,50,250,-5,5);

  return 0;
}

//-----------------------------------------------------------------------------
int TGausFitAlgNew::FillHistograms() {

  //  float level = 0.07;

  double           q1,  q2, ph1, ph2, chi2_1, chi2_2, t1c, t2c, dtc;
  TTofData        *dt, *data1, *data2;
  TDrsFitResults  *res, *r1,  *r2, *tr1, *tr2;
  ...
  tr1 = GetTrigFitResults(igr1);
  tr2 = GetTrigFitResults(igr2);

  t1c = T0(r1,fLevel)-T0(tr1,fLevel);
  t2c = T0(r2,fLevel)-T0(tr2,fLevel);
...
//-----------------------------------------------------------------------------
// the photopeak: both signals are large enough
//-----------------------------------------------------------------------------
    fHist.fDt [2]->Fill(fDt );
    fHist.fDtl[2]->Fill(fDtl);

    fHist.fDtc[2]->Fill(dtc);

  if (dtc < -3) {
    printf(" run = %6i event number = %i, T1 = %12.5f T2 = %12.5f TTR1 = %12.5f TTR2 = %12.5f\n",
	   TDrsGlobals::Instance()->GetRunNumber(),
    ...
    fHist.fDtcVsIch1->Fill(ich1,dtc);
    fHist.fDtcVsIch2->Fill(ich2,dtc);

    fHist.fDt2VsDfc->Fill(idfc,fDt);
    fHist.fDtcVsDfc->Fill(idfc,dtc);
					// debugging
    if (ich2 == 24) {
      fHist.fDt2VsIch1_24->Fill(ich1,fDt);
      fHist.fDtcVsIch1_24->Fill(ich1,dtc);

      fHist.fDt2VsDfc_24->Fill(idfc,fDt);
      fHist.fDtcVsDfc_24->Fill(idfc,dtc);
    }
    ...
    fHist.fDtVsSig12->Fill(sig12,fDt);
...
// Gaussian fit
//-----------------------------------------------------------------------------
int TGausFitAlgNew::Fit(const TTofData* Data) { 
...
//-----------------------------------------------------------------------------
  ...
  for (int i=max_bin; i>=0; i--) {
    if (Data->GetV1(i) > vmax*max_fraction) {
      max_bin = i;
    }
    if (Data->GetV1(i) > vmax*min_fraction) { // 2.) { 1=136.2
      min_bin = i;
    }
    else {
      break;
...
//-----------------------------------------------------------------------------
// fMinBin and fMaxBin define the leading edge fitting range
//-----------------------------------------------------------------------------
  res->fMinBin = min_bin;
  res->fMaxBin = max_bin;
...
//-----------------------------------------------------------------------------
    fMinuit->mnparm(0, "p0"   , 0.7    , 0.01, 0.6        , 2        , ierflg);
    fMinuit->mnparm(1, "p1"   , max_bin, 0.1 , max_bin-10, max_bin+10, ierflg);
    fMinuit->mnparm(2, "p2"   , 4.     , 0.1 , 2.        , 10.       , ierflg);
    ...
    res->fFun->SetParameter(0,0.7);
    res->fFun->SetParameter(1,max_bin);
    res->fFun->SetParameter(2,3.9);
    res->fFun->SetParLimits(2,1,10.);
    //  f->SetParError(2,0.);

    if (fNPar > 3) {
      res->fFun->SetParameter(3,0.01);
    }
