...
public:
   ...
   Osc(std::string ifname_0): ifname(ifname_0), tree(0)
   {
      if (oscBin.Open(ifname.c_str())) {
         cout<< "Successfully opened Osc binary file " << ifname.c_str() <<endl;
         return;
      }
   ...
   void Convert(Int_t entry1=0, Int_t entry2=0)
   {
      ...
      {
         cout<< "processing ientry " << ientry << " oscBin.Number = " << oscBin.Number() <<endl;
...
TTree* osc(const char* ifname, Int_t entry1=0, Int_t entry2=0)
{
